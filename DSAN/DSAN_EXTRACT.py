#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""DSAN/LMMD domain adaptation for the manuscript STFT datasets.

By default, train every directed source/target combination across the four
domains (4 * 3 = 12 transfer tasks).  ``--pairs`` can still be used to run a
smaller, explicitly selected subset.
"""
...
LAMBDA_LMMD_MAX, WARMUP_EPOCHS, LMMD_GAMMA = 0.10, 5, 10.0
...
def lmmd(source, target, source_labels, target_logits, confidence=0.70):
    """Local MMD for binary DSAN using hard source labels and soft target pseudo-labels."""
    n = min(len(source), len(target), len(source_labels), len(target_logits))
    if n < 2: return source.new_zeros(()), 0.0
    source, target = source[:n].float(), target[:n].float()
    source_labels = source_labels[:n].view(-1).long()
    with torch.no_grad():
        p1 = torch.sigmoid(target_logits[:n].float()).view(-1)
        target_prob = torch.stack([1.0 - p1, p1], dim=1)
        target_hard = target_prob.argmax(1)
        keep = target_prob.max(1).values >= float(confidence)
        source_onehot = torch.nn.functional.one_hot(source_labels, num_classes=2).float()
    ...
    kernel = sum(torch.exp(-distances / (2 * bandwidth * s * s)) for s in KERNEL_SCALES)
    kss, ktt, kst = kernel[:n, :n], kernel[n:, n:], kernel[:n, n:]

    loss, common = source.new_zeros(()), 0
    for c in range(2):
        target_mask = keep & (target_hard == c)
        if not (source_labels == c).any() or not target_mask.any():
            continue
        ws = source_onehot[:, c]; ws = ws / ws.sum().clamp_min(1e-6)
        wt = target_prob[:, c] * target_mask.float(); wt = wt / wt.sum().clamp_min(1e-6)
        loss = loss + (ws[:, None] * ws[None, :] * kss).sum()
        loss = loss + (wt[:, None] * wt[None, :] * ktt).sum()
        loss = loss - 2.0 * (ws[:, None] * wt[None, :] * kst).sum()
        common += 1
    if common == 0:
        return source.new_zeros(()), float(keep.float().mean().item())
    return loss / common, float(keep.float().mean().item())
...
def train_pair(source, target, seed, device, a):
    ...
    suffix = f"stft_dsan_{mode}_seed{seed}" + (f"_r{a.tgt_label_ratio:.3f}" if mode=="ssda" else "")
    ...
    model=DSAN(bool(a.use_gn)).to(device); opt=optim.Adam(model.parameters(),lr=a.lr,weight_decay=a.weight_decay)
    ...
    config={"method":"DSAN","source":source,"target":target,"seed":seed,"mode":mode,"ratio":a.tgt_label_ratio,
            "lambda_lmmd_max":a.lambda_lmmd_max,"warmup_epochs":a.warmup_epochs,"w_tgt_sup":a.w_tgt_sup,
            "pseudo_conf_thresh":a.pseudo_conf_thresh,
    ...
    if start==1 or a.force:
        with open(path["history"],"w") as f: f.write("epoch,total_loss,src_loss,tgt_loss,lmmd_loss,lambda_lmmd,pseudo_coverage,valid_loss,valid_f1,threshold,lr\n")

    begun=time.time(); log(f"[DSAN-{mode.upper()}] {source}->{target} seed={seed} S={len(s_tr)} T={len(t_tr)} Tlab={len(t_lab) if t_lab else 0}")
    for epoch in range(start,a.epochs+1):
        model.train(); si,ti,li=iter(sl),iter(tl),iter(ll) if ll else None; runloss={k:[] for k in ("all","src","tgt","lmmd","lam","coverage")}
        ...
        for i in bar:
            ...
            lam=0. if epoch<=a.warmup_epochs else a.lambda_lmmd_max*(2/(1+math.exp(-LMMD_GAMMA*progress))-1)
            opt.zero_grad(set_to_none=True)
            with autocast(enabled=scaler.is_enabled()):
                z,h=model(torch.cat([xs[:n],xt[:n]])); src=bce(z[:n],ys[:n])
                local_mmd, coverage=lmmd(h[:n],h[n:],ys[:n],z[n:],a.pseudo_conf_thresh)
                if xl is not None: zt,_=model(xl); tgt=bce(zt,yl)
                else: tgt=src.new_zeros(())
                loss=src+lam*local_mmd+a.w_tgt_sup*tgt
            ...
            if ema: ema.update()
            for k,v in zip(runloss,(loss,src,tgt,local_mmd,lam,coverage)): runloss[k].append(float(v.item() if torch.is_tensor(v) else v))
            bar.set_postfix(loss=f"{np.mean(runloss['all']):.4f}",lmmd=f"{np.mean(runloss['lmmd']):.4f}",keep=f"{np.mean(runloss['coverage']):.2f}")
        ...
        with open(path["history"],"a") as f:
            f.write(f"{epoch},{np.mean(runloss['all']):.6f},{np.mean(runloss['src']):.6f},{np.mean(runloss['tgt']):.6f},"
                    f"{np.mean(runloss['lmmd']):.6f},{np.mean(runloss['lam']):.6f},{np.mean(runloss['coverage']):.6f},"
                    f"{vloss:.6f},{vf1:.6f},{threshold:.6f},{opt.param_groups[0]['lr']:.8f}\n")
        ...
        torch.save(latest,path["latest"]); log(f"ep={epoch:03d}{'*' if improved else ' '} valid_f1={vf1:.4f} lmmd={np.mean(runloss['lmmd']):.4f} keep={np.mean(runloss['coverage']):.2f}")
    ...
    result={"mode":f"DSAN_{mode.upper()}","source":source,"target":target,"seed":seed,"best_epoch":ck["epoch"],
...
def parser():
    ...
    p.add_argument("--lambda-lmmd-max","--lambda-mmd-max",dest="lambda_lmmd_max",type=float,default=LAMBDA_LMMD_MAX)
    p.add_argument("--pseudo-conf-thresh",type=float,default=PSEUDO_CONF_THRESH); p.add_argument("--warmup-epochs",type=int,default=WARMUP_EPOCHS)
...
def main():
    a=parser().parse_args()
    if (a.epochs<1 or min(a.batch_src,a.batch_tgt)<2 or not 0<=a.tgt_label_ratio<=1
            or a.lambda_lmmd_max<0 or not 0<=a.pseudo_conf_thresh<1): raise ValueError("Invalid arguments")
    ...
    out=os.path.join(a.out_root,f"summary_dsan_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
