...
def checkpoint_candidates(task: str, method: str, seed: int) -> list[Path]:
    ...
    if method == "dann":
        root = MANUSCRIPT_ROOT / "DA_VS_FT_12_transfer/DANN" / task
        return [root / f"stft_ssda_seed{seed}_r{ratio}" / "best.pth" for ratio in ("0.10", "0.100", "0.1")]
    if method == "dan":
        roots = [MANUSCRIPT_ROOT / "DAN" / task, MANUSCRIPT_ROOT / "DA_VS_FT_12_transfer/DAN" / task]
        return [root / f"stft_dan_ssda_seed{seed}_r{ratio}" / "best.pth" for root in roots for ratio in ("0.100", "0.10", "0.1")]
    if method == "dsan":
        root = MANUSCRIPT_ROOT / "DA_VS_FT_12_transfer/DSAN" / task
        return [root / f"stft_dsan_ssda_seed{seed}_r{ratio}" / "best.pth" for ratio in ("0.100", "0.10", "0.1")]
