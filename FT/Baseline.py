"""Train source-only STFT baselines used to initialize FT_extract.

Best checkpoints are saved as:
  Baseline_FT/{domain}/stft_seed{seed}/{domain}_best.pth

The manuscript STFT .pt files already contain abs + log1p values.  This
script deliberately does not clamp/log1p them again; it only applies global
z-score statistics computed from the source-domain training set.
"""
